<template>
  <div class="dv-scroll-board" :ref="ref">
    ...
    <div
      ...
      :style="{
        height: `${
          height - (header.length ? mergedConfig.headerHeight : 0) - mergedConfig.columnMarginTop
        }px`,
...
<script lang="ts" setup>
    import autoResize from '../Container/autoResize.js'1
  ...
  import func from 'vue-editor-bridge'
    ...
    function calcData() {
          ...
          calcHeights()
          calcAligns()
          if (mergedConfig.value.autoPlay) {
            animation(true)
          }
        }
  function mergeConfig () {
    mergedConfig.value = deepMerge(deepClone(defaultConfig.value, true), props.config || {})
  }
  ...
  function  calcRowsData() {
          let { data, index, headerBGC, rowNum, columnIndex } = mergedConfig.value
          if (index) {
            data = data.map((row, i) => {
              row = [...row]
